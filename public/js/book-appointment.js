document.addEventListener("DOMContentLoaded", function () {
    ...
    function validateName() {
        ...
        if (name.length < 3) {
            document.getElementById("errmsgname").innerHTML = "Minimum 3 characters required";
            return false;
        }

        if (!pattern.test(name)) {
            document.getElementById("errmsgname").innerHTML = "Only letters and spaces allowed";
            return false;
        }
    ...
    function validateMobile() {
        ...
        if (!/^[6-9]\d{9}$/.test(mobile)) {
            document.getElementById("errmsg").innerHTML = "Enter valid 10 digit mobile number";
            return false;
        }
    ...
    function validateEmail() {
        ...
        if (!pattern.test(email)) {
            document.getElementById("errmsgEmail").innerHTML = "Enter valid email address";
            return false;
        }
    ...
    function loadProcedures(keyword = "") {
        ...
        fetch(API_BASE_URL + "/procedure-categories?q=" + encodeURIComponent(keyword))
            .then(res => res.json())
            .then(response => {
